...
class ANMA(object):
    ...
    **nb_kwargs
    ...
    def fit(self, X, EigenFile):
        ...
        EigenFile: le nom d fichier ou seront stockés les vecteurs et valeurs propres
        Returns
        -------
        self : object
        ...
        self._solve()
        # ecrit dans EigenFile les vecteurs propres et valeurs propres
        nbVector = len(self.eigenvalues_)
        for v in range(nbVector):
            vector = [elem[v] for elem in self.eigenvectors_ ]
            U.ecrit_eigenVectors(vector, self.eigenvalues_[v], EigenFile, v, v)

        self._dirty = False
        return self

    def transform(self, X, EigenFile):
        ...
        """
        if self._dirty:
            self.fit(X,EigenFile)
            self._dirty = False
            return # programme termine
        return self._animate(self.mode)

    def fit_transform(self, X, EigenFile):
        ...
        """
        self._dirty = True
        return self.transform(X,EigenFile)
